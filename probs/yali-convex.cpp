...
struct Point {
    ...
    Point operator - (const Point &rh) const {
        Point res;
        res.x = x - rh.x; res.y = y - rh.y;
        return res;
    }
    ll operator * (const Point &rh) {
        return (ll)x * rh.y - (ll)y * rh.x;
    }
    bool operator < (const Point &rh) { // polar order
        if (y > 0 && rh.y < 0) return true;
        if (y < 0 && rh.y > 0) return false;
        return *this * rh > 0;
    }
} a[N];
...
int prev(cint x) {
    ...
    if (it == s.begin()) return *std::prev(s.end());
    return *std::prev(it);
}
int next(cint x) {
    std::set<int>::iterator it = std::next(s.find(x));
    if (it == s.end()) return *s.begin();
    return *it;
}
void ins(int i, const bool f = true) {
    int x = rk[i];
    if (f) s.insert(x);
    const Point &p = a[sa[prev(x)]];
    const Point &q = a[sa[next(x)]];
    if (f) res += std::abs((p - a[i]) * (q - a[i]));
    else s.erase(x), res -= std::abs((p - a[i]) * (q - a[i]));
}
int main() {
    int m;
    io >> n >> m;
    int siz = sqrt((double)n * n / m);
    int c = 0;
    rep (i, 1, n) {
        io >> a[i].x >> a[i].y;
        sa[i] = i;
        if (++c > siz) bel[i] = bel[i - 1] + 1, c = 1;
        else bel[i] = bel[i - 1];
    }
    ...
    assert(n >= 3);
    l = 1, r = 3;
    res = std::abs((a[2] - a[1]) * (a[3] - a[1]));
    s.insert(rk[1]); s.insert(rk[2]); s.insert(rk[3]);
    rep (i, 1, m) {
        const Query &q = qwq[i];
        while (l > q.l) ins(--l, true);
        while (r < q.r) ins(++r, true);
        while (l < q.l) ins(l++, false);
        while (r > q.r) ins(r--, false);
        *q.ans = res;
    }
    rep (i, 1, m)
        io << ans[i] daze;
