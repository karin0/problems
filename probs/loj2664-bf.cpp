...
int n, d, k, a[N][D];
std::bitset<100> b[100000];
int main() {
    io >> n >> d >> k;
    if (k == 2) {
        re (i, 0, n) re (j, 0, d) if ((int)io & 1) b[i].set(j);
        re (i, 0, n) re (j, i + 1, n) if (((b[i] & b[j]).count() & 1) == 0)
            return io << (i + 1) << ' ' << (j + 1) << '\n', 0;
    } else if (n <= 2000) {
        re (i, 0, n) re (j, 0, d) a[i][j] = (int)io % k;
        re (i, 0, n) re (j, i + 1, n) {
            int s = 0;
            re (h, 0, d) s += a[i][h] * a[j][h];
            if (s % k == 0)
                return io << (i + 1) << ' ' << (j + 1) << '\n', 0;
        }
    } else
