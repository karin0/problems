...
cint N = 2002, mo = 1000000007;

int add(int a, cint b) {
    return (a += b) >= mo ? a - mo: a;
}
int n, m, a[N], g[N][N], h[N][N];
bool f[N];
int main() {
	io >> n >> m;
	rep (i, 1, n)
	...
	std::sort(a + 1, a + n + 1);
	int l = 1, r = n, c = n;
	while (c) (f[c--] = a[l] + a[r] >= m) ? --r : ++l;
    // crep (i, 1, n) ccc(i, f[i]);
    h[1][1] = h[1][0] = 1;
	rep (i, 2, n) rep (j, 0, i) {
        g[i][j] = j < i ? g[i - 1][j] + f[i] * j : 0;
        if (j) {
            int t = g[i - 1][j - 1] + f[i] * (i - j);
            if (t > g[i][j]) g[i][j] = t, h[i][j] = h[i - 1][j - 1];
            else if (t == g[i][j]) h[i][j] = add(h[i - 1][j - 1], h[i - 1][j]);
            else h[i][j] = h[i - 1][j];
        } else h[i][j] = h[i - 1][j];
        // ccc(i, j, f[i], g[i][j], h[i][j]);
		// g[i][j] = std::max(j ? g[i - 1][j - 1] + f[i] * (i - j) : 0, g[i - 1][j] + f[i] * j);
    }
    int *gn = g[n], *hn = h[n];
    int ga = gn[0], ha = hn[0];
    rep (i, 1, n) {
        if (gn[i] > ga) ga = gn[i], ha = hn[i];
        else if (gn[i] == ga) ha = add(ha, hn[i]);
    }
    io << ga << ' ' << ha daze;
