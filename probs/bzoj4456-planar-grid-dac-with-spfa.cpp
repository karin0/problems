...
int n, m, p, vx[N], vy[N], d[N], xb0, xb1, yb0, yb1;
struct Pos {
    int x, y;
    Pos() {}
...
struct Data {
    ...
    int uid;
    Data() {}
    Data(const Pos &_u, const int _uid) : u(_u), uid(_uid) {}
};
void spfa(const Pos s, const int sid) {
    static std::queue<Data> q;
    static bool inq[N];
    static int x, y, uid, vid, t, du;
    static Pos u;
    static Data dt;
    uid = id(xb0, yb0);
    for (x = xb0; x <= xb1; ++x, uid = t + m) {
        t = uid;
        for (y = yb0; y <= yb1; ++y, ++uid)
            d[uid] = inf;
    }
    d[sid] = 0;
    q.push(Data(s, sid));
