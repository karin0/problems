...
int n, m, c, b[N], h[N], fwd[N];
std::vector<int> a[N][N];
int main() {
    static int ks, kc, i, j, x;
    ...
    rep (kc, 1, ks) {
        ...
        rep (i, 1, n) {
            if (fwd[i] <= h[i])
                ...
                while (r - l > 1) {
                    x = (l + r) >> 1;
                    // printf("Case #%d, i = %d, l = %d, r = %d, x = %d\n",kc,i,l,r,x);
                    ni = i - x;
                    g::reset();
                    rep (j, 1, m)
                        g::arc(&g::g[n + j], g::t, b[j]);
                    re (j, 1, ni) if (fwd[j] <= m) {  // ******** 检查是否有学上
                        g::arc(g::s, &g::g[j], 1);
                        ed = a[j][fwd[j]].end();
