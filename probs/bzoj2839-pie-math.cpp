...
const int N = 1000002, mo = 1000000007;
...
ll binom(cint n, cint k) {
    return fac[n] * faci[k] % mo * faci[n - k] % mo;
}
int main() {
    int n, k, ans = 0;
    io >> n >> k;
    ...
    rep (i, 2, n)
        fac[i] = fac[i - 1] * i % mo;
    faci[n] = qpow(fac[n], mo - 2);
    per (i, n - 1, 2)
        faci[i] = faci[i + 1] * (i + 1) % mo;
    faci[0] = faci[1] = 1;
    ll t = 2;
    per (i, n - k, 0)
        ans = (ans + ((i & 1) ? -1 : 1) * binom(n - k, i) * t + mo) % mo, t = t * t % mo;
    //    ans = (ans + ((i & 1) ? -1 : 1) * binom(n - k, i) * (qpow(2, qpow(2, n - k - i, mo - 1), mo) - 1) + mo) % mo;
    io << ans * binom(n, k) % mo daze;
