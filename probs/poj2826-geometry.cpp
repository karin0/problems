...
struct point_t {
    ...
    bool operator == (const point_t &rh) const {
        return !dcmp(x, rh.x) && !dcmp(y, rh.y);
    }
...
bool point_t::on(const segment_t &l) const {
    return dcmp(vector_t(l.a, *this) * vector_t(*this, l.b)) == 0 && dcmp(l.a.x - x) * dcmp(x - l.b.x) >= 0 && dcmp(l.a.y - y) * dcmp(y - l.b.y) >= 0;  // *****
}
...
bool is_intersected(const segment_t &a, const segment_t &b) {
    return a.a.on(b) || a.b.on(b) || b.a.on(a) || b.b.on(a) || is_proper_intersected(a, b);  // ***
}
...
double solve(segment_t a, segment_t b) {
    ...
    // if (dcmp(ap.x, bp.x) == 0 || dcmp(p.x, ap.x) * dcmp(ap.x, bp.x) > 0) // ***
    // puts("do");
    if (is_intersected(segment_t(ap, point_t(ap.x, 40000)), b)) // *******
        return 0;
    // printf("%f >= %f\n", ap.y, p.y);
    if (dcmp(ap.y, p.y) < 0)
        return 0;
