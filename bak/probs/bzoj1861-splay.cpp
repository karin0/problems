...
struct Node {
    ...
    void *operator new (size_t) {
        static Node pool[N + N], *curr = pool;
        return curr++;
    }
} *mp[N];
...
void make_top(const int x) {
    mp[x]->cut();
    mp[x] = new Node(x, NULL);
    Node *o = select2(0);
    Node *p = o->succ();
    o->splay();
    p->splay(o);
...
int main() {
    ...
    while (m--) {
        ...
        switch (op[0]) {
            ...
            case 'B': make_bottom(x); break;
            case 'I': insert(x, io); break;
            case 'A': io.print(query_rk(x) - 1); break;
